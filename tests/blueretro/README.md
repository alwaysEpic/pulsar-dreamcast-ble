...
1. advertises the name our `PROFILE_XBOX` uses (`Xbox Wireless Controller`),
2. sends BlueRetro the exact `HID_REPORT_DESCRIPTOR_XBOX` our firmware serves,
...
- `test_pulsar_xbox_buttons_mapping` — every face/shoulder/system button (the issue).
- `test_pulsar_xbox_dpad_mapping` — hat switch → D-pad directions (`hat_to_ld_btns`).
- `test_pulsar_xbox_axes_scaling` — sticks (unsigned 0-65535, center 32768) and
...
- `test_pulsar_generic_buttons_probe` — `xfail`-tolerant characterization of the Generic
  profile (contiguous layout, Steam/PC target) on BlueRetro. XFAIL = mis-maps as
  expected; XPASS = it happens to work, revisit.

All Xbox assertions are at the `generic_input` stage (descriptor-parse correctness,
system-independent). A red Xbox test means issue #2 is reproduced — the harness
...
- The Generic profile's real target (Steam Deck / kernel / xpadneo) is a *different*
